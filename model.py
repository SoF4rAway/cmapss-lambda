...
class RUL_1D_CNN(nn.Module):
    ...
    def __init__(
        ...
        for i in range(num_blocks):
            ...
            # Padding to keep length stable if needed, but here we use 'same' padding logic
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Forward pass.
        Input x: (Batch, SeqLen, Features) -> (N, 30, 24)
        """
...
def validate_onnx(onnx_path: str, dummy_input: np.ndarray):
    ...
    print(f"ONNX validation successful. Output shape: {outputs[0].shape}")
    return outputs

if __name__ == "__main__":
    # Test model
    model = RUL_1D_CNN()
    test_input = torch.randn(8, 30, 24)
    ...
    try:
        validate_onnx("model.onnx", test_input.numpy())
    except ImportError:
